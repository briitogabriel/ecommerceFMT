    ...
    switch (type) {
      ...
      case 'minus':
        setCart(cart.map(item => item.id === id ? 
          item.quantidade > 1 ? { ...item, quantidade: item.quantidade - 1 } : item
          : item))
        break;
        ...
        <div className="col-md-8 d-flex flex-wrap justify-content-evenly">
          <div className="card-body">
            ...
            <div className="d-flex flex-wrap justify-content-evenly align-items-center" style={{ minWidth: '150px' }}>
              <button className="btn-qtd" onClick={() => changeAmount('minus', product.id)}><BiMinus /></button>
              <h5 className="card-title mx-2 my-0">{product.quantidade}</h5>
              <button className="btn-qtd" onClick={() => changeAmount('plus', product.id)}><BiPlus /></button>
            </div>
            <p className="card-text">R$ {product.preco.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
          </div>
        <AiOutlineDelete className='m-3' style={{cursor: 'pointer', color: 'red'}} onClick={() => changeAmount('delete', product.id)} />
        </div>
      </div>
    </div>
  )
